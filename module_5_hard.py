...
class Video:
    def __init__(self, title, duration, adult_mode=False):
        ...
        self.time_now = 0  # current time in seconds
        ...
        for video in self.videos:
            if video.title == title:
                ...
                for second in range(video.duration):
                    time.sleep(1)  # simulate watching each second
                    print(f"Просмотр: {second + 1} сек.")
                print("Конец видео.")
                video.time_now = 0  # сброс текущего времени после просмотра
                return
